...
func (ckmgr *CheckpointManager) doCheckpoint(vbno uint16, through_seqno_map map[uint16]uint64,
	...
	// Item 8: Manifests
	manifestsPair, err := ckmgr.collectionsManifestSvc.GetOngoingManifests(ckmgr.pipeline.Specification(), vbno)
	srcManifest := manifestsPair.Source
	tgtManifest := manifestsPair.Target
	if err != nil {
		ckmgr.logger.Warnf("%v got err %v when retrieving manifests", ckmgr.pipeline.Topic(), err)
	} else {
		ckmgr.logger.Infof("Checkpointing with srcManifest ID: %v tgtManifest ID: %v\n", srcManifest.Uid(), tgtManifest.Uid())
	}
	...
	newCkpt := metadata.NewCheckpointRecord(ckRecordFailoverUuid, through_seqno, ckRecordDcpSnapSeqno, ckRecordDcpSnapEndSeqno, ckptRecordTargetSeqno,
		uint64(filteredItems), uint64(filterFailed), srcManifest.Uid(), tgtManifest.Uid())
