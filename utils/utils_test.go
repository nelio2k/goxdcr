...
func TestStripXattrAndCompression(t *testing.T) {
	...
	_, err, releaseFunc := testUtils.ProcessUprEventForFiltering(uprEvent, dp)
	assert.Nil(err)
	defer releaseFunc()
	...
	_, err, releaseFunc2 := testUtils.ProcessUprEventForFiltering(uprEventCompressed, dp)
	defer releaseFunc2()
	...
	_, err, releaseFunc3 := testUtils.ProcessUprEventForFiltering(uprEventXattr, dp)
	assert.Nil(err)
	defer releaseFunc3()
	...
	_, err, releaseFunc4 := testUtils.ProcessUprEventForFiltering(uprEventXattrCompressed, dp)
	assert.Nil(err)
	defer releaseFunc4()
...
func TestProcessBinaryFile(t *testing.T) {
	...
	checkMap := map[string]interface{}{
		base.ReservedWordsMap["KEY"]: string(uprEvent.Key),
	}
	checkSlice, err := json.Marshal(checkMap)
	assert.Nil(err)

	retSlice, err, _ := testUtils.ProcessUprEventForFiltering(uprEvent, dp)
	assert.Nil(err)
