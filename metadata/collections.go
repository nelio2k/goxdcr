...
type collectionsTempObj struct {
	...
	UidTemp_    float64                `json:"uid"`
	ScopesTemp_ map[string]interface{} `json:"scopes"`
}

func newCollectionsTempObj() *collectionsTempObj {
	return &collectionsTempObj{
		ScopesTemp_: make(map[string]interface{}),
	}
}
...
func NewCollectionsManifestFromMap(manifestInfo map[string]interface{}) (CollectionsManifest, error) {
	...
	if uid, ok := manifestInfo["uid"].(float64); ok {
		tempObj.UidTemp_ = uid
	} else {
		return manifest, base.ErrorInvalidInput
	}

	if scopes, ok := manifestInfo["scopes"].(map[string]interface{}); ok {
...
func (c *CollectionsManifest) Load(collectionsTemp *collectionsTempObj) error {
	c.uid = uint64(collectionsTemp.UidTemp_)
	c.scopes = make(ScopesMap)
	for scopeName, scopeDetail := range collectionsTemp.ScopesTemp_ {
		scopeDetailMap, ok := scopeDetail.(map[string]interface{})
		if !ok {
...
func (c *CollectionsManifest) MarshalJSON() ([]byte, error) {
	collectionsTemp := newCollectionsTempObj()
	collectionsTemp.UidTemp_ = float64(c.uid)

	for scopeName, scope := range c.scopes {
		collectionsTemp.ScopesTemp_[scopeName] = scope.toScopeDetail()
	}
...
func NewScope(name string, scopeDetail map[string]interface{}) (Scope, error) {
	uidFloat, ok := scopeDetail[base.UIDKey].(float64)
	if !ok {
		return Scope{}, base.ErrorInvalidInput
	}
	collectionsDetail, ok := scopeDetail[base.CollectionsKey].(map[string]interface{})
	if !ok {
		return Scope{}, base.ErrorInvalidInput
	}
...
func (s *Scope) toScopeDetail() map[string]interface{} {
	...
	detailMap[base.UIDKey] = float64(s.Uid)
...
func NewCollectionsMap(collectionsDetail map[string]interface{}) (map[string]Collection, error) {
	collectionMap := make(CollectionsMap)

	for collectionName, detail := range collectionsDetail {
		colDetail, ok := detail.(map[string]interface{})
		if !ok {
			return nil, base.ErrorInvalidInput
		}

		uid, ok := colDetail[base.UIDKey].(float64)
...
func (c *CollectionsMap) toCollectionsDetail() map[string]interface{} {
	detailMap := make(map[string]interface{})

	for colName, collection := range *c {
		detail := make(map[string]interface{})
		detail[base.UIDKey] = float64(collection.Uid)
		detailMap[colName] = detail
	}
