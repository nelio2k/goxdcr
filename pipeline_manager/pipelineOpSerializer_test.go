...
func TestPipelineOpSerializerReinit(t *testing.T) {
	...
	serializer, pipelineMgr := setupBoilerPlateSerializer()
	repStatusPtr := &pipeline.ReplicationStatus{}
	pipelineMgr.On("GetOrCreateReplicationStatus", mock.Anything, mock.Anything).Return(repStatusPtr, nil).Times(1)
	pipelineMgr.On("StopPipeline", mock.Anything).Return(nil).Times(1)
	pipelineMgr.On("RemoveReplicationCheckpoints", mock.Anything).Return(nil).Times(1)
	pipelineMgr.On("Update", mock.Anything, mock.Anything).Return(nil).Times(1)
